...
export default {
  ...
  created(){
    this.$http.get(this.baseUrl+"mock/data.json").then((res)=>{
      console.log(res)
       this.options=res.data[0] //得到json里面的 每一项
    })
  },
  methods:{
    handleChangeCity(){
      this.magShow=true //当点击的时候让这个组件显示出来
    },
    handleProvince(value){  //省份的值
      this.options.forEach((res)=>{ //把每个城市遍历出来
        if(res.value===value){  //如果省份的值相等就给他赋值
